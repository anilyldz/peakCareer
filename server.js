  ...
  await retry(async (bail, attempt) => {
    try {
      ...
      const payment = {
        ...
        // Front endten parametre olarak sadece token alıp ödeme yapılıyor bu yüzden amountı nasıl göndereceğiz? ANIL

        amountMoney: {
          // the expected amount is in cents, meaning this is $1.00.
          // Buraya dikkat 100 aslında $1.00 demek o yüzden 100 dolar değil 1 dolar ödeme yapılıyor ANIL
          amount: '100',
          // If you are a non-US account, you must change the currency to match the country in which
          // you are accepting the payment.

          // Currency sabit kalabilir ANIL
          currency: 'USD',
        },
      };
...
// Token alarak çalıştığı için ileride başka ödeme yöntemi entegre etmemiz gerekirse
// Diğer ödeme yöntemlerindeki appId ve locationId'yi değiştirerek entegre olabiliriz ANIL
